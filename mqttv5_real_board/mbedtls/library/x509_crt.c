...
volatile int32_t real_board_x509_last_pem_ret = 0;
volatile int32_t real_board_x509_last_der_ret = 0;
volatile uint32_t real_board_x509_last_pem_use_len = 0;
volatile uint32_t real_board_x509_last_der_len = 0;
volatile uint8_t real_board_x509_last_format = 0;
                           ...
                           size_t buflen)
{
    ...
    real_board_x509_last_pem_ret = 0;
    real_board_x509_last_der_ret = 0;
    real_board_x509_last_pem_use_len = 0;
    real_board_x509_last_der_len = (uint32_t) buflen;
    real_board_x509_last_format = 0;
    ...
    if (buflen != 0 && buf[buflen - 1] == '\0' &&
        ...
        real_board_x509_last_format = 1U;
        buf_format = MBEDTLS_X509_FORMAT_PEM;
    }

    if (buf_format == MBEDTLS_X509_FORMAT_DER) {
        int der_ret = mbedtls_x509_crt_parse_der(chain, buf, buflen);
        real_board_x509_last_der_ret = der_ret;
        return der_ret;
    }
#else
    int der_ret = mbedtls_x509_crt_parse_der(chain, buf, buflen);
    ...
    if (buf_format == MBEDTLS_X509_FORMAT_PEM) {
        ...
        while (buflen > 1) {
            ...
            ret = mbedtls_pem_read_buffer(&pem,
                                          ...
                                          "-----END CERTIFICATE-----",
                                          buf, NULL, 0, &use_len);
            real_board_x509_last_pem_ret = ret;
            real_board_x509_last_pem_use_len = (uint32_t) use_len;
            ...
            real_board_x509_last_der_len = (uint32_t) pem.buflen;
            real_board_x509_last_der_ret = ret;

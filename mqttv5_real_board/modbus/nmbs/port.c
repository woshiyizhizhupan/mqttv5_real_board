...
int32_t read_socket(uint8_t* buf, uint16_t count, int32_t byte_timeout_ms, void* arg) {
    ...
    while (total < count) {
        ...
        if (getSn_RX_RSR(MB_SOCKET) == 0) {
            if (byte_timeout_ms >= 0 && elapsed_ms >= (uint32_t) byte_timeout_ms) {
        ...
        ret = recv(MB_SOCKET, buf + total, (uint16_t)(count - total));
        if (ret > 0) {
            total += (uint16_t)ret;
            elapsed_ms = 0;
            continue;
...
int32_t write_socket(const uint8_t* buf, uint16_t count, int32_t byte_timeout_ms, void* arg) {
    ...
    while (total < count) {
        int32_t ret = send(MB_SOCKET, (uint8_t*)buf + total, (uint16_t)(count - total));
        if (ret > 0) {
            total += (uint16_t)ret;
            elapsed_ms = 0;
            continue;

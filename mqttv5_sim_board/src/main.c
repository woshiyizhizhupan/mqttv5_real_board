...
int socket_recv(void *ctx, unsigned char *buf, size_t len)
{
    ...
    int32_t ret = recv(sock_fd, (uint8_t *)buf, len);
    switch (ret)
    {
    case SOCKERR_SOCKSTATUS:
        break;
...
#define MQTT_PACKET_BUF_SIZE 2200
#define MQTT_JSON_BUF_SIZE 1800
...
static int mqtt_publish_json(mqtt_topic_index_t topic_index, const char *json, uint8_t qos_level)
{
    return mqtt_publish_json_to_topic_ex(system_config->mqtt_server.topics[topic_index], json, qos_level, 0U);
}
...
static void mqtt_publish_response(const char *id, const char *cmd, uint8_t ok, int code, const char *message, cJSON *data)
{
    ...
    if (cJSON_PrintPreallocated(root, mqtt_json_buf, sizeof(mqtt_json_buf), 0))
        mqtt_publish_json(MQTT_TOPIC_EVENT_UP_INDEX, mqtt_json_buf, system_config->mqtt_server.qos[MQTT_TOPIC_EVENT_UP_INDEX]);
    cJSON_Delete(root);
}
...
static uint8_t mqtt_is_legacy_response_payload_envelope(cJSON *root)
{
    ...
    return (prefix[0] == 0xFEU) && (prefix[1] == 0xA5U) && (prefix[2] == 0x81U);
}
...
static void mqtt_publish_debug(const char *event, const char *message)
{
    ...
    if (cJSON_PrintPreallocated(root, mqtt_json_buf, sizeof(mqtt_json_buf), 0))
        mqtt_publish_json(MQTT_TOPIC_DEBUG_UP_INDEX, mqtt_json_buf, system_config->mqtt_server.qos[MQTT_TOPIC_DEBUG_UP_INDEX]);
    cJSON_Delete(root);
}
...
static void mqtt_publish_telemetry(void)
{
    ...
    sim_board_business_append_telemetry(root);

    if (cJSON_PrintPreallocated(root, mqtt_json_buf, sizeof(mqtt_json_buf), 0))
        mqtt_publish_json(MQTT_TOPIC_TELEMETRY_UP_INDEX, mqtt_json_buf, system_config->mqtt_server.qos[MQTT_TOPIC_TELEMETRY_UP_INDEX]);
    cJSON_Delete(root);
}

static void mqtt_publish_sim_event(void)
{
    cJSON *root = cJSON_CreateObject();
    if (root == NULL)
        return;

    if (sim_board_business_publish_periodic_event(root) &&
...
static void mqtt_handle_command_json(uint8_t *payload_in, int32_t payloadlen_in)
{
    char id[40];
    char error[80];
    cJSON *root;
    ...
    legacy_payload_envelope = mqtt_is_legacy_payload_envelope(root);
    if (legacy_payload_envelope && mqtt_is_legacy_response_payload_envelope(root))
    {
        cJSON_Delete(root);
        return;
    }
    ...
    {
        if (sim_board_business_handle_legacy_command(root, "legacy_frame", &legacy_data, &legacy_code, &legacy_message, &legacy_reboot_requested))
        {
            if (legacy_payload_envelope)
                mqtt_publish_legacy_payload_envelope_response(root,
                                                              legacy_data,
                                                              legacy_code == 0,
                                                              legacy_code,
                                                              legacy_message);
    ...
    else if (strcmp(cmd, "get_status") == 0)
    {
        ...
        sim_board_business_append_status(data);
        sim_board_ota_append_status(data);
        mqtt_publish_response(id, cmd, 1, 0, "ok", data);
    }
    ...
    else if ((strcmp(cmd, "sim_set") == 0) || (strcmp(cmd, "sim_event") == 0))
    {
        if (sim_board_business_handle_json_command(root, cmd, &module_data, &module_code, &module_message))
            mqtt_publish_response(id, cmd, module_code == 0, module_code, module_message, module_data);
        else
            mqtt_publish_response(id, cmd, 0, 404, "unknown simulation command", NULL);
    }
    ...
    {
        if (sim_board_ota_handle_command(root, cmd, &module_data, &module_code, &module_message, &module_reboot_requested))
        {
            mqtt_publish_response(id, cmd, module_code == 0, module_code, module_message, module_data);
    ...
    else if (sim_board_business_handle_legacy_command(root, cmd, &legacy_data, &legacy_code, &legacy_message, &legacy_reboot_requested))
    {
...
uint8_t mqtt_connect()
{
    ...
    // Subscribe command and OTA topics; other topics are published only.
    mqtt_subscribe((char *)system_config->mqtt_server.topics[MQTT_TOPIC_CMD_DOWN_INDEX], system_config->mqtt_server.qos[MQTT_TOPIC_CMD_DOWN_INDEX]);
    mqtt_subscribe((char *)system_config->mqtt_server.topics[MQTT_TOPIC_OTA_INDEX], system_config->mqtt_server.qos[MQTT_TOPIC_OTA_INDEX]);
...
mqtt_state_t state = MQTT_STATE_INIT;
    ...
    case MQTT_STATE_CONNECTED:
        ...
        if (rc == PUBLISH)
        {
            ...
            if (rc == 1)
            {
                ...
                if (qos <= 2)
                    mqtt_handle_command_json(payload_in, payloadlen_in);
                else
